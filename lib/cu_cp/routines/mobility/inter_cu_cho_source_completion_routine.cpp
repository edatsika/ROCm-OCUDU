...
#include "inter_cu_cho_source_completion_routine.h"
#include "ocudu/ran/cause/ngap_cause.h"

using namespace ocudu;
using namespace ocucp;

inter_cu_cho_source_completion_routine::inter_cu_cho_source_completion_routine(
...
void inter_cu_cho_source_completion_routine::operator()(coro_context<async_task<void>>& ctx)
{
  CORO_BEGIN(ctx);

  {
    ...
    // Collect non-winning intra-CU target UE indices for release after we're done.
    for (const auto& candidate : source_ue->get_cho_context()->candidates) {
      if (!candidate.is_inter_cu() && candidate.target_ue_index != ue_index_t::invalid &&
          candidate.target_ue_index != source_ue_index) {
        intra_cu_targets_to_release.push_back(candidate.target_ue_index);
      }
    }
  ...
  // Step 1: Query CU-UP for PDCP SN status (per TS 38.423, source sends SN Status Transfer AFTER HandoverSuccess).
  ...
  // Step 2: Send SN Status Transfer to the winning target CU-CP via Xn.
  if (bearer_mod_resp.success && fill_status_transfer() && winner_xnap != nullptr) {
    winner_xnap->handle_sn_status_transfer_required(status_transfer);
  } else {
    logger.warning("ue={}: \"{}\" SN Status Transfer skipped (bearer mod failed or no xnap)", source_ue_index, name());
  }

  // Step 3: Release non-winning intra-CU target UE contexts.
  release_idx = 0;
  while (release_idx < intra_cu_targets_to_release.size()) {
    if (ue_mng.find_du_ue(intra_cu_targets_to_release[release_idx]) == nullptr) {
      ++release_idx;
      continue;
    }
    release_cmd                      = {};
    release_cmd.ue_index             = intra_cu_targets_to_release[release_idx];
...
void inter_cu_cho_source_completion_routine::fill_e1ap_bearer_modification_request_pdcp_sn_query()
{
  ...
  const auto& pdu_sessions = ue_mng.find_du_ue(source_ue_index)->get_up_resource_manager().get_pdu_sessions();
  for (const auto& pdu_session_id : pdu_sessions) {
    ...
    const up_pdu_session_context& pdu_session =
        ue_mng.find_du_ue(source_ue_index)->get_up_resource_manager().get_pdu_session_context(pdu_session_id);
...
bool inter_cu_cho_source_completion_routine::fill_status_transfer()
{
  ...
  for (const auto& pdu_session_modified : bearer_mod_resp.pdu_session_resource_modified_list) {
    const up_pdu_session_context& pdu_session_ctx =
        ue_mng.find_du_ue(source_ue_index)->get_up_resource_manager().get_pdu_session_context(
    ...
    for (const e1ap_drb_modified_item_ng_ran& drb_modified : pdu_session_modified.drb_modified_list_ng_ran) {
      ...
      drb_item.drb_status_ul.ul_count.hfn =
          drb_modified.pdcp_sn_status_info->pdcp_status_transfer_ul.count_value.hfn;
