...
inline bool asn1_to_handover_request(xnap_handover_request& request, const asn1::xnap::ho_request_s& asn1_request)
{
  ...
  if (request.is_conditional_handover &&
      asn1_request->ch_oinfo_req.ie_exts.cho_time_based_info_present) {
    request.cho_timeout = std::chrono::seconds{
        asn1_request->ch_oinfo_req.ie_exts.cho_time_based_info.cho_ho_win_dur};
  }
