...
const pucch_resource* pucch_resource_manager::ue_reservation_guard::reserve_sr_resource()
{
  ...
  const unsigned sr_cell_res_id = res_params.get_sr_cell_res_idx(ue_bwp_cfg.ul.pucch.sr_res_id);
...
const pucch_resource* pucch_resource_manager::ue_reservation_guard::reserve_csi_resource()
{
  ...
  // Check if the wanted resource is already allocated to another UE in this slot.
  const unsigned csi_cell_res_id = res_params.get_csi_cell_res_idx(ue_bwp_cfg.ul.periodic_csi_report->pucch_res_id);
...
pucch_harq_resource_alloc_record pucch_resource_manager::ue_reservation_guard::reserve_next_harq_res_available()
{
  ...
  const auto              res_set_cfg_id = ue_bwp_cfg.ul.pucch.res_set_cfg_id;
...
pucch_resource_manager::ue_reservation_guard::reserve_harq_resource_by_res_indicator(unsigned d_pri)
{
  ...
  const unsigned max_pri = parent->cell_cfg.is_pucch_f0_and_f2()
                               ? res_set_size + (ue_bwp_cfg.ul.periodic_csi_report.has_value() ? 2U : 1U)
                               : res_set_size;
  ...
  if (parent->cell_cfg.is_pucch_f0_and_f2() and d_pri >= res_set_size) {
    if (ResourceSetId == 0) {
      if (d_pri == res_set_size) {
        return &cell_pucch_cfg.resources[res_params.get_sr_cell_res_idx(ue_bwp_cfg.ul.pucch.sr_res_id)];
      }
      return &cell_pucch_cfg
                  .resources[res_params.get_csi_f0_cell_res_idx(ue_bwp_cfg.ul.periodic_csi_report->pucch_res_id)];
    }
    // Resource Set ID 1.
    if (d_pri == res_set_size) {
      return &cell_pucch_cfg.resources[res_params.get_sr_f2_cell_res_idx(ue_bwp_cfg.ul.pucch.sr_res_id)];
    }
    return &cell_pucch_cfg.resources[res_params.get_csi_cell_res_idx(ue_bwp_cfg.ul.periodic_csi_report->pucch_res_id)];
  }

  // Check first if the wanted PUCCH resource is available.
  const unsigned cell_res_id =
      res_params.get_res_set_cell_res_idx<ResourceSetId>(ue_bwp_cfg.ul.pucch.res_set_cfg_id, d_pri);
  auto& res_rnti = ctx.ues_using_pucch_res[cell_res_id];
  if (res_rnti != rnti_t::INVALID_RNTI and res_rnti != rnti) {
    return nullptr;
  }
...
bool pucch_resource_manager::ue_reservation_guard::release_harq_resource()
{
  ...
  for (unsigned r_pucch = 0; r_pucch != res_set_size; ++r_pucch) {
    const unsigned cell_res_id =
        res_params.get_res_set_cell_res_idx<ResourceSetId>(ue_bwp_cfg.ul.pucch.res_set_cfg_id, r_pucch);
    auto& res_rnti = ctx.ues_using_pucch_res[cell_res_id];
