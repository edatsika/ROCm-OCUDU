        ...
        pusch_td_resource_indices_per_slot)
{
  ...
  // In FDD, we return a list with 1 value only, which applies to all DL slots.
  ...
  fmt::print("\n k1 list=[ \t");
  for (unsigned i = 0, sz = dl_data_to_ul_ack.size(); i != sz; ++i) {
      fmt::print("{}\t", dl_data_to_ul_ack[i]);
  }
  fmt::print("]\n");
  ...
  for (unsigned sl_idx = 0; sl_idx != nof_slots; ++sl_idx) {
    if (has_active_tdd_dl_symbols(tdd_cfg_common.value(), sl_idx)) {
      // If there is no k2 candidate for this DL slot, then we can use min k1.
