...
bool rrc_ue_impl::handle_rrc_handover_preparation_info(byte_buffer pdu)
{
  ...
  // Parse capabilities into the decoded struct so capability checks (e.g. CHO support) work on the target.
  if (auto caps = get_capabilities(ue_capabilities_list, logger); caps.has_value()) {
  ...
  if (request.is_cho_preparation) {
    ...
    if (request.non_crit_ext.has_value() && !request.non_crit_ext->master_cell_group.empty()) {
      ...
      if (cell_group_cfg.unpack(bref) == asn1::OCUDUASN_SUCCESS &&
          cell_group_cfg.sp_cell_cfg_present && cell_group_cfg.sp_cell_cfg.recfg_with_sync_present) {
