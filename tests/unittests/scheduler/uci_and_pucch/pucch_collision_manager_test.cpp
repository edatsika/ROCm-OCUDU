...
static cell_configuration
make_test_cell_configuration(const pucch_resource_builder_params& ded_resources = pucch_resource_builder_params{},
...
{
  const auto cell_cfg = make_test_cell_configuration(pucch_resource_builder_params{
      .res_set_0_size           = 8,
      .res_set_1_size           = 8,
...
protected:
  pucch_collision_manager_rg_test() :
    cell_cfg(make_test_cell_configuration()),
...
private:
  ...
  {
    std::vector<pucch_collision_info> infos;
    infos.reserve(cell_cfg.init_bwp.ul.pucch.resources.size());
    for (const auto& resource : cell_cfg.init_bwp.ul.pucch.resources) {
...
TEST_F(pucch_collision_manager_rg_test, alloc_fills_grants_in_ul_res_grid)
{
  ...
  for (unsigned i = 0; i != cell_cfg.init_bwp.ul.pucch.resources.size(); ++i) {
